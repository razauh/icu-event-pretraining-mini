...
from icu_pretrain.training.pretrain import train_model, TrainingInterruptedException
from icu_pretrain.data.dataset import EncodedDataset

def create_synthetic_processed_dir(tmp_path: Path) -> Path:
...
def get_default_config(processed_dir: Path) -> dict:
    return {
        "experiment": {
            "name": "test_pretrain_run",
            "seed": 42,
        },
        "data": {
            "dataset": "eicu_demo",
...
def test_pretraining_run_uninterrupted(tmp_path: Path):
    processed_dir = create_synthetic_processed_dir(tmp_path)
    config = get_default_config(processed_dir)
    run_dir = tmp_path / "run_uninterrupted"
    results = train_model(config, processed_dir, run_dir, resume="no")
...
def test_gradient_accumulation(tmp_path: Path):
    processed_dir = create_synthetic_processed_dir(tmp_path)
    config = get_default_config(processed_dir)
    config["pretraining"]["gradient_accumulation_steps"] = 4
    run_dir = tmp_path / "run_grad_accum"
    results = train_model(config, processed_dir, run_dir, resume="no")
    assert results is not None

def test_resumable_pretraining_equivalence(tmp_path: Path):
    processed_dir = create_synthetic_processed_dir(tmp_path)
    config = get_default_config(processed_dir)
    ...
    np.random.seed(42)
    random.seed(42)
    res_uninterrupted = train_model(config, processed_dir, run_dir_uninterrupted, resume="no")
    run_dir_interrupted = tmp_path / "run_interrupted"
    torch.manual_seed(42)
    np.random.seed(42)
    random.seed(42)
    with pytest.raises(TrainingInterruptedException):
        train_model(config, processed_dir, run_dir_interrupted, resume="no", interrupt_after_batches=2)
    with open(run_dir_interrupted / "state.json", "r", encoding="utf-8") as f:
        state_data = json.load(f)
    assert state_data["status"] == "interrupted"
    res_resumed = train_model(config, processed_dir, run_dir_interrupted, resume="auto")
...
def test_pretraining_missing_data(tmp_path: Path):
    processed_dir = tmp_path / "empty_processed"
    processed_dir.mkdir()
    config = get_default_config(processed_dir)
    with pytest.raises(FileNotFoundError):
        train_model(config, processed_dir, tmp_path / "run", resume="no")

def test_pretraining_nan_loss(tmp_path: Path, monkeypatch):
    processed_dir = create_synthetic_processed_dir(tmp_path)
    config = get_default_config(processed_dir)
    run_dir = tmp_path / "run_nan"
    import icu_pretrain.training.pretrain as pretrain_mod
    monkeypatch.setattr(pretrain_mod, "compute_mlm_loss", lambda *args, **kwargs: torch.tensor(float("nan"), requires_grad=True))
    with pytest.raises(ValueError, match="NaN loss encountered"):
        train_model(config, processed_dir, run_dir, resume="no")
...
def test_incompatible_checkpoint(tmp_path: Path):
    ...
    with pytest.raises(TrainingInterruptedException):
        train_model(config, processed_dir, run_dir, resume="no", interrupt_after_batches=1)
    ...
    with pytest.raises(ValueError, match="checkpoint artifact hashes are incompatible"):
        train_model(config, processed_dir, run_dir, resume="auto")
